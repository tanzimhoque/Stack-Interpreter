...
def execute(program, stack, memory):
	...
	while pc < len(program):
		...
		if line == INSTR_ADD:
			...
			output = stack.stack[stack.pointer - 1]
			memory.push(output)
			pc += 2
		elif line == INSTR_POP:
			output = stack.pop();
			pc += 2
		elif line == INSTR_SWITCH:
			temp = stack.stack[stack.pointer - 1]
			stack.stack[stack.pointer - 1] = stack.stack[stack.pointer - 2]
			stack.stack[stack.pointer - 2] = temp
			pc += 2
